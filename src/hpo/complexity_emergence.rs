...
impl MolecularBonding {
    ...
    /// Try to form a molecule from atoms
    /// 
    /// Bonds form based on valence compatibility (from archetype patterns)
    pub fn form_molecule(&mut self, atoms: &[ComplexAtom]) -> Option<ComplexMolecule> {
        if atoms.len() < 2 {
            return None;
        }
        
        // Check if atoms can bond based on valence
        ...
        for i in 0..atoms.len() {
            ...
            for j in (i + 1)..atoms.len() {
                ...
                if self.can_bond(atom_i.valence, atom_j.valence) {
                    bonds.push((i, j));
                    used.insert(i);
                    used.insert(j);
                    break;
    ...
    /// Check if two atoms can bond based on valence
    fn can_bond(&self, valence1: usize, valence2: usize) -> bool {
        // Simplified valence bonding
        // Real chemistry would be more complex
        if valence1 == 0 || valence2 == 0 {
            return false;
        }
        
        // Can bond if at least one has available valence
        let available1 = 8 - valence1; // Assuming noble gas configuration
        let available2 = 8 - valence2;
...
impl ComplexityEmergence {
    ...
    pub fn try_form_molecules(&mut self) {
        // Get all atoms
        let atoms: Vec<_> = self.atomic_formation.atoms.values().cloned().collect();
        ...
        for chunk in atoms.chunks(3) {
            if let Some(molecule) = self.molecular_bonding.form_molecule(chunk) {
                // Molecule formed
                _ = molecule;
            }
        }
