    ...
    pub fn apply(&self, node: &mut OctreeNode) {
        ...
        if let Some(next_layer) = self.layer.next() {
            ...
            for (i, amp) in node.field_data.density_amplitudes.iter_mut().enumerate() {
                ...
                let scale = if amp.magnitude() > 0.001 { new_mag / amp.magnitude() } else { 1.0 };
...
impl CosmicSequence {
    ...
    fn initialize_attractors(&mut self) {
        // Violet at origin
        self.attractors.push(LayerAttractor::new(CosmologicalLayer::Violet, [0.0, 0.0, 0.0]));
        
        // Indigo slightly offset
        self.attractors.push(LayerAttractor::new(CosmologicalLayer::Indigo, [10.0, 0.0, 0.0]));
        
        // Blue at center
        self.attractors.push(LayerAttractor::new(CosmologicalLayer::Blue, [0.0, 0.0, 0.0]));
        
        // Green spreads out
        self.attractors.push(LayerAttractor::new(CosmologicalLayer::Green, [50.0, 0.0, 0.0]));
        
        // Yellow - veil position
        self.attractors.push(LayerAttractor::new(CosmologicalLayer::Yellow, [100.0, 0.0, 0.0]));
        
        // Orange - galactic scale
        self.attractors.push(LayerAttractor::new(CosmologicalLayer::Orange, [200.0, 50.0, 0.0]));
        
        // Red - solar scale
        self.attractors.push(LayerAttractor::new(CosmologicalLayer::Red, [300.0, 100.0, 50.0]));
        
        // Layer 7 - individual entities spread
        self.attractors.push(LayerAttractor::new(CosmologicalLayer::Layer7, [100.0, 100.0, 50.0]));
    }
    ...
    fn apply_attractors(&mut self, field: &mut HolographicFieldState) {
        ...
        for (i, attractor) in self.attractors.iter_mut().enumerate() {
            ...
            if activation > 0.01 {
                ...
                self.statistics.max_attractor_strength = self.statistics.max_attractor_strength
