...
impl MatterEmergence {
    ...
    pub fn derive_particle_from_field(&mut self, field_data: &FieldNodeData, position: Position3D) -> Option<Particle> {
        ...
        let dominant = archetype_pattern.iter()
            .enumerate()
    ...
    pub fn update(&mut self, dt: Float) {
        ...
        let dead_particles: Vec<ParticleId> = self.particles.iter()
...
mod tests {
    ...
    fn test_particle_type_derivation() {
        ...
        matches!(ptype, ParticleType::Proton | ParticleType::Neutron | ParticleType::Custom(_));
        
        // Low mass
        let light = [0.01; 22];
        let ptype2 = ArchetypeParticleDerivation::derive_particle_type(&light);
        matches!(ptype2, ParticleType::Photon | ParticleType::Neutrino | ParticleType::Electron);
    }
