...
impl FreeWillTerm {
    ...
    pub fn apply(&mut self, node_data: &mut FieldNodeData, _position: &[Float; 3], _time: Float) {
        ...
        for (density_idx, &perturbation) in perturbations.iter().enumerate() {
            ...
            let u1: Float = self.rng.sample(uniform);
            let u2: Float = self.rng.sample(uniform);
            let imag_perturbation = (-2.0 * u1.abs().ln()).sqrt()
        ...
        node_data.spectrum_position =
            (node_data.spectrum_position + spectrum_shift).clamp(0.0, 2.0);
    }
...
impl LoveTerm {
    ...
    pub fn apply(&mut self, node: &mut OctreeNode) {
        ...
        let gradient_magnitude =
            (gradient[0].powi(2) + gradient[1].powi(2) + gradient[2].powi(2)).sqrt();
...
impl LightTerm {
    ...
    pub fn apply(&mut self, node: &mut OctreeNode, dt: Float) {
        ...
        let acceleration = self.propagation_speed.powi(2) * laplacian - self.damping * velocity;
    ...
    fn calculate_laplacian(&self, node: &OctreeNode, dx: Float, dy: Float, dz: Float) -> Float {
        ...
        let neighbor_estimate =
            center[0].sin() * center[1].sin() * center[2].sin() * current_energy;
    ...
    fn get_prev_energy(&self, position: &[Float; 3]) -> Float {
        ...
        for state in self.prev_states.iter() {
            let dist = ((position[0] - state.position[0]).powi(2)
                ...
                + (position[2] - state.position[2]).powi(2))
            .sqrt();
            if dist < min_dist {
                min_dist = dist;
...
impl PrimalDistortionTerms {
    pub fn new(config: &UnifiedFieldConfig) -> Self {
        PrimalDistortionTerms {
            ...
            love: LoveTerm::new(
                config.love_attraction_strength,
                config.love_attraction_range,
            ),
...
impl UnifiedFieldEquation {
    ...
    fn evolve_node(&mut self, node: &mut OctreeNode, dt: Float) {
        ...
        self.distortions
            .free_will
            .apply(&mut node.field_data, &position, self.time);
    ...
    fn collect_high_coherence_nodes(
        &self,
        node: &OctreeNode,
        threshold: Float,
    ...
    fn collect_stats(
        &self,
        node: &OctreeNode,
        total_coherence: &mut Float,
        total_energy: &mut Float,
        count: &mut usize,
        max_coherence: &mut Float,
    ) {
