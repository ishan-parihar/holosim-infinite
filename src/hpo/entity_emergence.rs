...
impl EntityEmergence {
    ...
    fn apply_node_to_field(&self, field: &mut HolographicFieldState, node: &FieldStructureNode) {
        ...
        // Update coherence
        field.root.field_data.coherence = (field.root.field_data.coherence + node.coherence * 0.01).min(1.0);
    ...
    fn handle_deaths(&mut self) {
        let to_remove: Vec<usize> = self.entities
            .iter()
    ...
    pub fn get_entity_positions(&self) -> Vec<[Float; 3]> {
        self.entities.values().filter(|e| e.active).map(|e| e.position).collect()
    }
    
    pub fn get_entities_for_rendering(&self) -> Vec<EntityRenderData> {
        self.entities.values().filter(|e| e.active).map(|e| EntityRenderData {
            id: e.id,
            position: e.position,
