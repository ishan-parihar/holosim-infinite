...
const REALM_DATA: [(&str, Color32); 7] = [
    ...
    ("Indigo - Intelligent Infinity", Color32::from_rgb(75, 0, 130)),
    ("Blue - Love/Logos", Color32::from_rgb(0, 0, 255)),
    ("Green - Light/Love", Color32::from_rgb(0, 128, 0)),
    ("Yellow - Dimensional Architecture", Color32::from_rgb(255, 255, 0)),
...
impl EntityInspectorEnhanced {
    ...
    fn show_entity_info(&mut self, ui: &mut egui::Ui, entity: &SubSubLogos) {
        ui.horizontal(|ui| {
            ...
            ui.label(RichText::new(format!("{}", entity.entity_type.display_name())).color(Color32::LIGHT_BLUE));
        });
        ui.separator();
    ...
    fn show_consciousness(&self, ui: &mut egui::Ui, entity: &SubSubLogos) {
        ui.collapsing("🧠 Consciousness & Polarization", |ui| {
            ...
            ui.add(egui::ProgressBar::new((entity.consciousness_level as f32).min(1.0)).show_percentage());
            ...
            ui.label(format!("Polarity Orientation: {}", 
                polarity_text));
    ...
    fn show_spectrum(&self, ui: &mut egui::Ui, entity: &SubSubLogos) {
        ui.collapsing("🌈 Spectrum Configuration", |ui| {
            ui.label(format!("Spectrum Position: {:.4}", entity.spectrum_position()));
            ui.label(format!("Space/Time Ratio: {:.4}", entity.space_time_ratio));
            ui.label(format!("Time/Space Ratio: {:.4}", entity.time_space_ratio));
            ui.label(format!("Veil Active: {}", entity.spectrum_access.veil_active));
        });
        ui.add_space(5.0);
    }
    ...
    fn show_realms_section(&self, ui: &mut egui::Ui, entity: &SubSubLogos) {
        ui.collapsing("🔮 7-Layer Holonic Architecture", |ui| {
            ...
            let realm_intensities = [
                ...
                (entity.yellow_realm.dimensional_architecture.dimensions.len() as f32 / 10.0).min(1.0),
                (entity.archetype_activations.iter().sum::<f64>() / 22.0) as f32,
            ];
            
            for (i, (name, color)) in REALM_DATA.iter().enumerate() {
                ui.horizontal(|ui| {
                    ui.label(RichText::new(format!("{}:", name.split(" - ").next().unwrap())).size(11.0));
                    ui.add_sized([150.0, 15.0], egui::ProgressBar::new(realm_intensities[i].min(1.0))
    ...
    fn show_archetypes_section(&self, ui: &mut egui::Ui, entity: &SubSubLogos) {
        ui.collapsing("⭐ 22 Archetypes (Archetypical Mind)", |ui| {
            let active_count = entity.archetype_activations.iter()
                .filter(|&&v| v > 0.1).count();
            ...
            for (archetype_idx, activation, name) in archetype_list {
                if activation > 0.01 { // Only show significant activations
                    ui.horizontal(|ui| {
                        ui.label(RichText::new(format!("#{}", archetype_idx + 1)).size(10.0));
                        ui.add_sized([200.0, 12.0], egui::ProgressBar::new(activation as f32)
    ...
    fn show_energy_section(&self, ui: &mut egui::Ui, entity: &SubSubLogos) {
        ui.collapsing("⚡ Energy & Vibrational State", |ui| {
            ...
            ui.label(format!("Total Energy: {:.2}", entity.potential_energy + entity.kinetic_energy));
            ...
            ui.label(format!("  {:.2} Hz", entity.current_state.vibrational_state.frequency));
            ...
            ui.label(format!("  {:.2}", entity.current_state.vibrational_state.amplitude));
            ...
            ui.label(format!("  {:.2}", entity.current_state.vibrational_state.coherence));
        });
        ui.add_space(5.0);
    }
    ...
    fn show_evolution_section(&self, ui: &mut egui::Ui, entity: &SubSubLogos) {
        ui.collapsing("🔄 Evolution & Density", |ui| {
            ...
            ui.label(format!("Evolutionary Rate: {:.2}x", entity.evolutionary_rate));
            
            ui.add_space(5.0);
            
            ui.label(format!("Current Density: {}", format_density(entity.current_density.clone())));
...
mod tests {
    ...
    fn test_archetype_names() {
        ...
        assert_eq!(ARCHETYPE_NAMES[21], "22. The Choice (XXII) - Unifying Archetype");
    }
