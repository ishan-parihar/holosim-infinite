...
impl ViewConfig {
    ...
    pub fn new(camera_target: Vec3, zoom_target: f32, rotation_target: f32, transition_duration: f32) -> Self {
        ViewConfig {
            camera_target,
            zoom_target,
            rotation_target,
...
impl ViewSystem {
    ...
    pub fn switch_view(&mut self, view_type: ViewType, current_position: Vec3, current_zoom: f32, current_rotation: f32) {
    ...
    pub fn update_transition(&mut self, delta_time: Duration, current_time: Duration) -> Option<(Vec3, f32, f32)> {
        if let Some(target_view) = self.state.target_view {
            ...
            let position = lerp_vec3(&self.state.start_position, &config.camera_target, self.state.transition_progress);
            let zoom = lerp(self.state.start_zoom, config.zoom_target, self.state.transition_progress);
            let rotation = lerp_angle(self.state.start_rotation, config.rotation_target, self.state.transition_progress);
...
fn lerp_angle(a: f32, b: f32, t: f32) -> f32 {
    let diff = b - a;
    let normalized_diff = ((diff + std::f32::consts::PI) % (2.0 * std::f32::consts::PI)) - std::f32::consts::PI;
    a + normalized_diff * t
}
