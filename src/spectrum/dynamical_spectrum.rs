...
impl DynamicalSpectrum {
    ...
    pub fn record_choice(&mut self, entity_id: u64, orientation: PolarityOrientation, magnitude: f64, step: usize) {
        if let Some(trajectory) = self.trajectories.get_mut(&entity_id) {
            let previous_orientation = trajectory.choice_history.last()
                .map(|c| c.orientation);
            ...
            if let Some(prev) = previous_orientation {
                if prev != orientation {
                    match orientation {
                        PolarityOrientation::ServiceToOthers => self.statistics.transitions_to_sto += 1,
                        PolarityOrientation::ServiceToSelf => self.statistics.transitions_to_sts += 1,
                        _ => {}
                    }
                }
            }
    ...
    pub fn get_position(&self, entity_id: u64) -> Option<f64> {
        self.trajectories.get(&entity_id).map(|t| t.current_position)
    }
    
    /// Get smoothed spectrum position for entity
    pub fn get_smoothed_position(&self, entity_id: u64) -> Option<f64> {
        self.trajectories.get(&entity_id).map(|t| t.get_smoothed_position())
    }
    ...
    pub fn get_density(&self, entity_id: u64) -> Option<Density> {
        ...
        let density = match position as usize {
            0..=10 => Density::First(crate::evolution_density_octave::density_octave::Density1SubLevel::Quantum),
            11..=20 => Density::Second(crate::evolution_density_octave::density_octave::Density2SubLevel::Cellular),
            21..=30 => Density::Third,
